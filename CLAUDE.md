...
* **素のテーブルへ値を書き込む前に、その値が何を守っているのかを確かめる。** 素の定数は
  たいてい**その先にある本当の制約を守るための門番**でしかない。守りの実体が Lua の外
  （ネイティブ / サーバー）にあるなら、書き換えても制約は消えず、**門番だけが外れる**。
  外した門番は自分で持ち直すこと。検出は `docs/check_vanilla_writes.py`（CI の `bundle`）。
...
実機での動作確認では変えた画面を必ず開くので、**そのときに撮ってもらい、同じ PR で差し替える**
（別 Issue に切り出すと、確認とは別にもう一度ゲームを起動する手間が生じて後回しになる）。

* **渡すタイミング**: 動作確認の観点を渡すとき、**撮影リストも一緒に渡す**。
* **撮影リストに書くこと**: 保存先のファイル名（`images/` 配下のパス） / 何が変わったのか /
  撮る手順（どこを開き、どの設定を ON にするか） / README のどの行から参照されているか。
* **回収するとき**: 動作確認の結果と一緒に画像を受け取り、`images/` へ置いて同じ PR に commit する。
  PR 本文の「動作確認」にも画像を貼る（確認の証跡を兼ねる）。
* **alt テキストは PR の中で直す**。撮った画像を見て、実際の画面と食い違っていないかも確かめる。
* **その PR の中で撮れないときだけ Issue にする**（その場で入れないコンテンツなど）。書く内容は撮影リストと同じで、
  PR 本文に Issue 番号を書く。
* **見た目を変えていない PR では撮らない。** 内部実装だけの変更は対象外。
