...
ネイティブ）の使い方は [docs/vanilla_api.json](docs/vanilla_api.json) に固定してある。
IMC 側のパッチで関数が消える・引数が変わっても**構文チェックは通り、実機でその機能を
触った瞬間にだけ落ちる**ので、機械で見られるようにしたもの。手順は
[docs/VANILLA_API.md](docs/VANILLA_API.md)。
...
**まだ書き写しのまま残っている 4 件がある**（Issue #94。素の途中で表示を絞る作りで、
「素を呼んでから加工」に素直に落ちないもの）。書き換えるまでの間、素が変わったことに
気付けるよう、**写し元の本文のハッシュを `docs/vanilla_api.json` の `copies` に記録して
`--verify-client` で照合している**（[docs/VANILLA_API.md](docs/VANILLA_API.md) の
...
## アドオンを新しく追加するときは docs の手順に従う

アドオンを 1 本足すと、**コードのほかに 5 か所へ登録が要る**（registry のエントリと説明、
build_manifest の連結順、配布 README の一覧と更新履歴）。どれか 1 つ抜けると
「一覧に出ない」「毎回設定が消える」という形になり、実機で触るまで気付けない。
手順・触るファイルの一覧・過去に踏んだ罠は [docs/NEW_ADDON.md](docs/NEW_ADDON.md) にまとめてある。
スキル `new-addon` からも同じ手順を辿れる（[.claude/skills/](.claude/skills/) は
**skills だけ追跡している**。個人設定が入る `.claude/` の他のファイルは無視のまま）。

**同じ罠を踏み直さないこと。** 特に `<key>_on_teardown` は**機能を一度も ON にしていない
利用者にも 1 回呼ばれる**、`g.setup_hook` の控えは**素の関数 1 つにつき 1 本しか持てない**、
の 2 つは実際に踏んでいる。

## レベル上限が解放されたときは docs の手順に従う

Lv 上限が上がると新しい女神・レイド・チャレンジの段が増え、**毎回同じ場所を触る**ことになる。
手順・触るファイルの一覧・過去に踏んだ罠は [docs/LEVEL_CAP_UPDATE.md](docs/LEVEL_CAP_UPDATE.md)
にまとめてある。スキル `lv-cap-update` からも同じ手順を辿れる。

**同じ調査を繰り返さないこと。** ID の一次資料は**ローカルのゲーム本体**で、
upstream の `_client/**` は取り込み時点のスナップショットなので新パッチのデータが入っていない。
