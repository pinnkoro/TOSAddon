...
-- Addons Menu(norisan さん系のメニューボタン)へ相乗りする。**これが主な入口。**
--
-- **_G["norisan"]["MENU"] は共有の待ち合わせ場所**で、ここへ {name, func, icon} を入れると
-- メニューを持っているアドオン(Nexus Addons P など)が拾って並べてくれる。
--
-- **Addons Menu を持つアドオンが入っていない環境では、ここは何も出さない。**
-- そのときの入口はマーケットの「イコル診断」ボタン(market.lua)だけになる。
-- 以前は画面に自前のフローティングボタンを出していたが、常時出ているのが
-- 邪魔なので外した(素のクライアントにはアドオンがチャットコマンドを足す仕組みが
-- 無いので、代わりの入口をチャットで足すことはできない)。
