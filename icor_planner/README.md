...
* 入力欄の右の **`最低` / `平均` / `最大`** で、目標値の目安を入れられます。
  ...
  | **相殺** | `最大` だけが働き、**ステータスの割合が 50% になる値**を入れます（Lv560 なら 8,400）。`最低` / `平均` は灰色 |
...
* **`最低` / `平均`** を押すと、カテゴリ「特殊装備強化素材」と武器 / 防具のサブカテゴリを選び、
  **条件欄を空にしてから**セットの 4 オプションと下限をまとめて入れます（検索の実行は素の検索ボタンで）
...
* パネルの上部には**オススメの検索条件**がボタンで並びます。押すと素の条件検索へ
  オプションと下限値が入ります（検索の実行までは行いません）。同じオプションは 1 つだけ出します
...
* イコルの付け替えそのものは行いません。刻印ページの管理と付け替えは
  [Goddess Icor Manager](../goddess_icor_manager/README.md) の役割です。
