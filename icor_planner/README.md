...
![「試算」タブ。上に「差し替えを全部戻す」「略語 ON」と「差し替える部位」のボタンが並ぶ。左は 今のイコル（右上に「自分で組む」）・インベントリ・マーケット（今の検索結果）の候補が略語のオプション付きで「試す」ボタンと並ぶ。右は差し替えた部位の一覧で、自分で組んだイコル（値: 枠ごと / 値: 平均。「探す」「組み直す」付き）、インベントリから試してリロールしたイコル、マーケットから試したイコル（「探す」「リロール」付き）が並び、その下に今と差し替え後の値と差、あとオプション 0 → 3、今の装備から変えるなら が出ている](images/06-simulate-tab.png)
...
**マーケットから試したイコルにも `探す` が付きます。** 下限はそのイコルに付いている値なので、
「これと同じオプションで、値がこれ以上の品」を探します（インベントリから試したイコルには付きません）。

**差し替えはキャラごとに保存され、次に開いたときにそのまま読み込まれます。**
「上半身はこれ、下半身はこれ」と組んでおけば、毎日マーケットを開いて `探す` を押すだけで確認できます。
組んだイコルとマーケットから試したイコルは、マーケットの評価パネルの一番上にも「試算のイコルで探す」として並びます
（診断ウィンドウを開かなくても、マーケットを見ながら同じ条件で探せます）。
...
* **差し替えはキャラごとに保存します**（`icor_planner.json` の `trial_swaps`）。やめるときは `×` か
  `差し替えを全部戻す` で戻してください。マーケットの出品は、押した時点のオプションを控えます
  （ページを移ると出品の中身が入れ替わるため）
...
![マーケットの横に出る評価パネル。上から 目標プリセット・「診断を開く」、並び順・「略語 ON」、「試算のイコルで探す」（メイン武器1(組) / 上半身(組) / メイン武器2(市) の 3 件が、略語のオプションと「探す」ボタン付きで並ぶ）、「オススメのセットで探す」のセット一覧（最低 / 平均 のボタン付き）、その下に評価された出品が割合と値段つきで並ぶ](images/07-market-panel.png)

**試算タブで自分で組んだイコル・マーケットから試したイコルがあれば、評価パネルの一番上に
「試算のイコルで探す」**が出ます。`探す` を押すと、そのイコルのオプションと下限（試算に使った値）を
まとめて入れて検索します。差し替えは保存されるので、ここは次にマーケットを開いたときも並んでいます。

```
試算のイコルで探す（押すとまとめて検索します）
  上半身(組) : 力 / 皮相殺 / 全防具 / 体力          [探す]
  下半身(市) : クリ発 / 体力 / 皮相殺 / 中型相殺    [探す]
```

`(組)` は自分で組んだイコル、`(市)` はマーケットから試したイコルです。
...
* **v1.2.0**
  * **試算タブの差し替えを保存するようにしました。** キャラごとに保存し、次に開いたときにそのまま読み込みます。
    「上半身はこれ、下半身はこれ」と組んでおけば、毎日マーケットで探し直せます
  * **マーケットから試したイコルにも `探す` を付けました。** 下限はそのイコルに付いている値
    （同じオプションで、値がそれ以上の品を探します）。マーケットの評価パネルの一番上の
    「試算のイコルで探す」にも、自分で組んだイコルと並んで出ます
  * 試算タブで、イコル名のレベルが `[Lv560] [Lv.560]…` と二重に出ていたのを直しました。
    自分で組んだイコルの行は `(値: 平均)` のように、値の見込みだけを出すようにしました
