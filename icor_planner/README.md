...
**この数と、目標の一覧の「必要オプション」の列は、下の「オススメのイコル構成」と同じ計算**です
（1 枠は一番上の段の平均値で数えます）。以前は一覧だけ別の簡易な計算で、オススメと食い違っていました。
...
「更新するイコル」を選んでいないときは、次のとおり**部位の種類ごとの理想の 1 形**です。

診断タブの目標の一覧の下に、**目標を満たすには武器イコル / 防具イコルにそれぞれ何を何個ずつ
載せればよいか**を出します。**部位の種類ごとの理想の 1 形**です（今のイコルに合わせた形ではありません）。

```
オススメのイコル構成  平均値で計算  あとオプション 7 個 / 最低値なら 9 個
...
* `×N` はその部位のイコル N 個に載せる、`(今 M)` は今その値以上で載っているイコルの数です。
  足りている項目は緑、足りない項目は橙です。「あとオプション N 個」は足りない分の合計です
...
![「試算」タブ。上に「差し替える部位」のボタンが並び、左に 今のイコル・インベントリ・マーケット（今の検索結果）のイコルが「試す」ボタン付きで並ぶ。右は差し替えた結果が出る欄](images/06-simulate-tab.png)
...
3. 右に**今と差し替え後**を並べます
   * 合計の目標 … 現在値（と ％）
   * 各部位の目標 … 載っている部位数
   * あとオプション（診断タブの見出しと同じ数）
   * 差は、良くなったら緑・悪くなったら赤です
4. 右の差し替えの一覧の **`×`** でその部位だけ戻し、**`差し替えを全部戻す`** で全部戻します
5. 右の一番下に、**差し替えた後の「更新するイコルのオススメ」**（診断タブと同じもの）が出ます。
   試したイコルに替えたら、残りの更新するイコルに何を載せればよいかが分かります
...
* **差し替えは保存しません**（その場で試すためのもの）。マーケットの出品は、押した時点の
  オプションを控えます（ページを移ると出品の中身が入れ替わるため）
* 武器の持ち替え側は別の部位です（メイン武器1 を替えても、メイン武器2 は替わりません）

### マーケット

![マーケットの横に出る評価パネル。上から 目標プリセット・「診断を開く」・並び順、「更新するイコルのセットで探す」のセット一覧（最低 / 平均 のボタン付き）、「オススメの検索条件」、その下に評価された出品が割合つきで並ぶ](images/07-market-panel.png)

**マーケットの評価パネルの一番上に「セットで探す」**が出ます（診断タブのオススメのイコル構成の、
イコル 1 個ぶんの中身）。
