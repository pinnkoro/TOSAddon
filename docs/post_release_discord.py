...
def build_messages(notes: str, link: str | None) -> list[str] | None:
...
def self_test() -> int:
    ...
    msgs = build_messages(template, "https://example.com/r")
    cases.append(("リンクを末尾に付ける", msgs is not None and msgs[-1].endswith("🔗 https://example.com/r")))
...
def main() -> int:
    ...
    if tag:
        ...
        print(__doc__)
        return 2
    msgs = build_messages(notes, link)
    if msgs is None:
    ...
    if dry_run:
        for n, m in enumerate(msgs, 1):
            print(f"===== {n}/{len(msgs)}（{discord_len(m)} 文字）=====\n{m}\n")
        return 0
    ...
    for n, m in enumerate(msgs, 1):
        post(webhook, m)
        print(f"  投稿 {n}/{len(msgs)}（{discord_len(m)} 文字）")
        if n < len(msgs):
