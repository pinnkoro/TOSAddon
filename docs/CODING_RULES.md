...
* 素に見当たらない名前は既定で NG。ただし素の exe の登録表に名前が在るネイティブ関数は
  自動で `native_exe` になり、NG にならない（[VANILLA_API.md](VANILLA_API.md)「exe の登録表との照合」）。
  それ以外は実在しないとは限らない（本家アドオンが定義するものなど）ので、
  確かめたうえで `EXPECTED_NOT_IN_CLIENT` か
