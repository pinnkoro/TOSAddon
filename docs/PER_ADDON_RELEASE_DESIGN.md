...
| 4 | ~~リリース基盤の N 個対応~~ **2 本ぶんは完了**（§3-3 / §3-4 / §3-5、Issue #192） | タグ・採番・検証・CI が全部 1 アドオン固定だった |
...
> ここは**調査した時点の状態**。§3-3〜§3-5 のとおり Issue #192 で一般化したので、
> 現在は [release.yml](../.github/workflows/release.yml) と
> [addon_targets.py](addon_targets.py) が `addons.json` の全エントリを見る。

* `release-nexus.yml`（当時）は `file == "nexus_addons_p"` 決め打ち、
...
### 3-3. タグ設計 — **実装済み（Issue #192）**

> 下表の形で入った。実装は [addon_targets.py](addon_targets.py)（`addons.json` から
> タグ名・アセット名・`.ipf` のファイル名を導く）と
> [.github/workflows/release.yml](../.github/workflows/release.yml)。
> 既存の `v1.0.0`〜`v2.10.0` は Nexus Addons P の過去の記録なのでそのまま残してある
> （今後の採番だけ `<file>-vX.Y.Z` になる）。
...
### 3-4. リリースワークフロー — **実装済み（Issue #192）**

`release-nexus.yml` を「`addons.json` の全エントリをループする」形に一般化する。
→ [.github/workflows/release.yml](../.github/workflows/release.yml) として入った。
公開対象の判定は [plan_release.py](plan_release.py)（手元でも同じものを流せる）、
実際の公開は matrix で 1 アドオンずつ。
...
### 3-5. 採番ルール — **一般化済み（Issue #192）**

* 現行の「main では版を上げない / `release-prep/**` でだけ採番」を **全エントリに一般化**する。
  → [check_version_freeze.py](check_version_freeze.py) が `addons.json` の全エントリについて
  3 箇所を見るようになった（エントリの追加・削除そのものも変更として検出する）。
  `addon.json`（§3-2 のメタデータ集約）はまだ入れていないので、今は `addons.json` +
  各アドオンの `00_header.lua` + `.ipf` ファイル名を見ている。
...
| 6 | リリース基盤の N 対応（ワークフロー / 採番 / 検証 / CI）**2 本ぶんは完了（Issue #192）** + `verify_ipf.py` をバイト一致検査へ強化（未） | `release-prep` での空撃ち | 2,5 |
