...
| `python docs/vanilla_api.py --verify-client` | **要る** | **手元で。PR を出す前だけ**（ビルドや実機確認のたびには流さない） | 一覧に記録した素の事実が、今のクライアントと合っているか |
| `python docs/vanilla_api.py --update` | 素の事実を更新するなら要る | 一覧を作り直すとき | — |

## `--verify-client` は前回 OK から何も変わっていなければ省略する

数 GB の `.ipf` から素の Lua を取り出すので重い。一方で結果を左右する入力は次の 3 つしか無いので、
前回 OK だったときの指紋を `docs/.vanilla_api_verified.json`（`.gitignore` 済み）に控え、
一致していれば照合を飛ばして `OK: ... 省略` を返す。

* 素のクライアントの `.ipf`（名前・サイズ・更新日時）… Steam の更新で変わる。
  `_` で始まるアドオンの `.ipf` は数えないので、テスト用の `-dev.ipf` を置き換えても照合し直しにはならない
* 一覧（`vanilla_api.json`）… 新しい素の API を使い始めて `--update` したら変わる
* 検査の実装（`vanilla_api.py` / `ipf_crypt.py`）… `COPIES` や除外リストを足したら変わる

**控えるのは OK のときだけ**（NG を控えると、直したつもりで飛ばされて緑に見える）。
照合し直したいときは `--force` を付ける。
...
* できない
  ...
  * 戻り値・副作用の変化。ここは実機で確かめるしかない（`g.vlog` で詳細ログを出す）
    （**例外**: 下の「書き写しの見張り」に載せた関数だけは、本文が変わったかを見ている）

## 書き写しの見張り（Issue #94）

素を呼ばずに**中身を書き写している**置換方式フックが 4 件ある。上の検査は
「**こちらが呼んでいる** API の名前と引数」を見るものなので、**呼んでいないこれらは
守れない**。素の中身だけが変わっても名前も引数も変わらず素通りする。

そこで写し元の**本文のハッシュ**を `vanilla_api.json` の `copies` に記録し、
`--verify-client` で照合する。素が変わったらそこで落ちる。

| コマンド | 何を見るか |
| --- | --- |
| `--check` | 写しの定義が src に在るか / 記録が一覧に在るか（素は要らない） |
| `--verify-client` | **写し元の本文が変わっていないか** |
| `--update` | 書き換える前に照合し、変わっていたら止まる（`--accept-client-changes` で承知のうえ取り込む） |

* **本文そのものは持たない。** 素の Lua は再配布できないので、記録するのは
  ハッシュと行数だけ。比較の前にコメントは落とし（文字列は残す）、改行と字下げは
  正規化する。**素の Lua は CRLF** なので、ここを正規化しないと関数の終わりを
  取り違える（実際 28 行の関数を 2234 行と数えた）。
* 対象は `docs/vanilla_api.py` の `COPIES` に並べる。**素を呼ぶ形へ書き換えたら
  そこから外すこと**。外し忘れは `--check` が落とす（もう無い写しを見張り続けないため）。
...
* `--update` は、素の Lua にも無い記号について exe に在るかを `in_exe` に記録する
  （素の Lua が使っている `native` にも記録する）。**`external` には記録しない** —
  拾えるのは名前だけなので、`utf8.codes` の `codes` のような短い名前は無関係な文字列に
  当たりうる。記録すると、その偶然の一致が外れただけで理由付きの記号まで落ちてしまう。
