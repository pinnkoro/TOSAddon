...
* **（次回リリース）**
  ...
  * Mini Addons: **「アイテム破片化の枠を拡張し、耳飾りを細かく絞り込む」を追加しました**
    ...
    * **「条件で選択」ボタン**が付きます。**残したい クラス / ランク / Lv を何行でも並べて**、
      そのどれにも当てはまらない耳飾りだけをまとめて選択できます（選ぶだけで、
      破片化そのものは今までどおり自分で実行ボタンを押します）。判定は
      **オプション 3 つのうち 1 つでも当てはまれば残す**、Lv は**その値以上**の意味です。
      クラス / ランク / Lv はそれぞれ「指定なし」にできます。
    * 並べた条件は **名前を付けて保存でき、後から読み込めます**（プリセット）。
