...
* **（次回リリース）**
  * Vakarine Equip: **街以外のすべてのマップで装備を着脱してしまう不具合を修正。**
    ...
    フィールドや旧ダンジョンでも着脱が走っていた。このチェックは今後
    **週間ボスレイド（ボス協同戦）のマップでも着脱するかどうか**だけを決める。
    インスタンスダンジョン（レイド / ミッション）はこの設定に関係なく従来どおり作動し、
    フィールドや旧ダンジョンでは作動しない。マップの判定は ID の直書きではなく
    マップの `Keyword` で行うので、マップが追加されても漏れない。
