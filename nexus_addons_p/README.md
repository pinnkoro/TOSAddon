...
* **（次回リリース）**
  * Muteki: **バフのエフェクトを選べるようにしました**。バフごとの設定に `エフェクト` ボタンが増え、
    「バフ開始時」と「重複が最大になったとき」のエフェクトを、一覧から選ぶか名前を直接入れて決められます。
    **どちらもチェックで個別に ON / OFF できます**（重複が最大のときのエフェクトを OFF にしても、
    効果音は今までどおり鳴ります）。
    **エフェクトの大きさ**も変えられます。その場で出して見た目を確かめられる **`試し再生`** も付けました。
    **既定は従来どおり**（開始時 `F_sys_TPBOX_great_300` を大きさ 6.0、
    重複が最大のとき `F_pattern025_loop` を大きさ 1.5）なので、設定を触らなければ今までと同じです。
    開始時のエフェクトが出ていない場合は、一覧から別のエフェクトに変えられます。
  * Muteki: **エフェクトの「種類」を選べるようにしました**。バフごとに
    **通常エフェクト**（キャラに出す）と **UIエフェクト**（Muteki の枠の上に描く）を切り替えられます。
    使える名前が種類ごとにまったく別なので、**一覧には選んだ種類の候補だけ**を出し、
    名前と大きさも種類ごとに別々に憶えます。
  * Muteki: **「エフェクト中は透明度を自動で上げる」**を足しました。システム設定の
    「自分のエフェクト透明度」を下げていると Muteki のエフェクトも薄くなってしまうので、
    出している間だけ透明度を戻します（戻す前にマップ移動やクライアント終了が起きても、
    次の起動時に元の値へ戻します）。透明度を下げたまま通知だけ見たいときは、
    UIエフェクトの方を選んでください。

* **v2.11.0**
  * Mini Addons: 週間ボスレイドの**ランキングのデータが保存されていなかった**のを直しました。
    取得を最後まで終えると「保存しました」「処理が完了しました」と出るのに、ランキングを表示しようとすると
    「ランキングデータが未取得です」と言われる状態でした。保存先のフォルダ（`addons\mini_addons_p\`）が
    作られておらず、保存に失敗しても何も言わずに進んでいたためです。
    保存先を設定と同じフォルダ（`addons\_nexus_addons_p\<アカウントID>\mini_addons_log.dat`）へ移し、
    保存できなかったときはそう伝えるようにしました。
    **個別版 Mini Addons で取得したデータも、初回に 1 回だけ引き継ぎます**。
    設定のバックアップ / 復元の対象にも入れました。
    ランキングを表示しようとして、選んでいる週のデータだけが無いときは、
    どの週を探したのか、どの週なら保存されているのかを出すようにしました。
  * Mini Addons: 週間ボスレイドの **data ボタンで出るメニューを見やすくしました**。
    「four weeks」「This week」の見出しが押せてしまっていたので見出しをやめ、各行の頭に
    「今週 / 4週分」を付けて色分けしました。よく使う「今週」を上に、それぞれの先頭に「全職業」を置いています。
    メニューの一番下に **「4週より古いデータを自動で削除」** の切り替えを足しました（**既定は OFF**）。
    ON にすると、次にデータを保存するときから、今週を含めて 4 週より古い週のデータを消します
    （保存データは取得するたびに増え続け、ランキングの週タブを切り替えるたびに丸ごと読み直しているため）。
    切り替えた時点では何も消しません。
  * Easy Buff: メシ屋を開いたときに**自動で食べるプリセットを、キャラごとに選べるようにしました**。
    設定画面の左端のチェックを、今ログインしているキャラの分として覚えます（タイトルの右にキャラ名が出ます）。
    プリセットの名前と料理は今までどおり全キャラ共通です。まだ選んでいないキャラは、これまでの設定のまま動きます。
  * Easy Buff: 自動実行のプリセットを選んでいると、**店のどのボタンを押しても自動実行のプリセットを食べていた**のを直しました。
    押したボタンのプリセットを食べます。

* **v2.10.0**
  * Indun Panel: **パネルの行にマウスを載せたときに、ツールチップが出たり消えたりして
    点滅していたのを直しました**（チャレンジの行の「左クリック: チャレンジマップの1週間分の
    スケジュール表示」で特に目立っていました）。
    展開したパネルは 1 秒ごとに中身を描き直しており、そのたびに同じツールチップを
    入れ直していたため、1 秒周期で消えてはまた出るという動きになっていました。
    **中身が変わったときだけ**入れ直すようにしています（入場券の所持数のように
    実際に数が変わるものは、これまでどおり更新されます）。
  * Market Favorite Rebuild: 受領箱の**「販売取り消し」の行に「再出品」ボタン**を足しました。
    「受け取る」の左隣に出ます。押すと受け取って販売タブを開き、**出品したときと同じ単価・個数・
    販売期間**を入れた状態にします。**登録は押しません**ので、そのときの最低価格を見てから
    自分で押せます。
    あわせて、ツールチップの**販売単価と販売個数を 3 桁ごとに区切って**表示するようにしました。
  * Mini Addons: **ダンジョン入場ウィンドウの右側に残っていた、見えないクリック判定を消せるように**
    しました（設定「フレーム関連」の新しい項目。既定は OFF）。素のウィンドウは枠が中身より大きく、
    レイドでは消えている「倍数モード」の箱のぶんまでクリックを飲んでいたため、
    窓の右側の何も無いところを押しても後ろのキャラクターや地面を触れませんでした。
    ON にすると枠を中身ぴったりへ詰めるので、その部分を押すと後ろへ届くようになります。
  * 内部整理: 複数のアドオンで使う部品（設定ファイルの読み書き・詳細ログ・ウィンドウの土台・
    ESC の扱い）を共通の置き場へまとめました。**画面の動きは変わりません。**

* **v2.9.1**
  * Indun Panel: **チャレンジマップの予定表が 1 日先へずれていた**のを修正しました。
    覚えている予定と違うマップに入ったときは予定を覚え直すので、大きくずれていた場合も
    次にチャレンジマップへ入った時点で直ります（予定どおりのときは設定を書き込みません）。
    あわせて、**「フィールドでも表示」が OFF だと予定をまったく覚えられなかった**のも直しました。

* **v2.9.0**
  * **Lv560 のレイド「偽りの輝翼」「堕落した審判の翼」のハードに対応**しました。
    * Indun Panel: 2 つのレイドの行に **HARD ボタンとハードの入場回数**を出します。
    * Indun List Viewer: 2 つのレイドを**ハード列にも出します**（既存の設定では表示 ON で追加されます）。
    * Quickslot Operate: ハードでも属性に合わせたポーションへ差し替えます。
    * Mini Addons: エンブレム・アークの着け忘れお知らせが、この 2 つのハードでも出るようにしました。
      あわせて、**ズメイのハードでお知らせが出ていなかった**のも直しました。

* **v2.8.1**
  * Indun Panel: **分裂特異点 520 の `USEor` ボタンを押すと、分裂特異点の券ではなく
    ライマラの入場券を買って使ってしまう**のを修正しました。ショップの品目が差し替わり、
    520 の券が TOS イベントショップから無くなっていたのが原因です。チャレンジ 520 も
    同じ理由でベリオラの券を買う状態だったため、あわせて直しています。520 の段のボタンは
    **`USE`（手持ちの券を使うだけ）** になり、購入枠の表示も出なくなります。
    ほかのボタンも、**ショップの品目がその入場券でなければ買わない**ようにしました。
  * Indun Panel: **焔の記憶の `HARD` ボタンを押しても入場できない**のを修正しました。
    ゲーム側で入場ボタンの仕組みが変わり、アドオンから渡す情報が足りていませんでした。
* **v2.8.0**
  * Mini Addons: グループチャットやささやきを選んだとき、**宛先の名前がチャット入力欄から
    下へ落ちて表示される**ことがあったのを修正しました。「チャット入力フレームにボタン追加」
    が OFF のときに起きていました。
  * Mini Addons: アイテム破片化で、**たくさん選んでから「破片へ変換」を押すと
    クライアントが落ちる**のを修正しました。枠を広げる機能が、素の「一度に実行できる
    枚数」の歯止めまで外してしまっていたのが原因です。一度に実行できるのは素と同じ
    25 個までとし、それを超えて選んでいるときは超過分の選択を外して、何個だけ実行するかを
    お知らせします。**超えたぶんは、実行が終わって一覧が更新された時点で自動的に
    選び直す**ので、「破片へ変換」を押し直すだけで続きを実行できます。
  * Mini Addons: アイテム破片化に**「自動」を追加しました**。ON にすると、25 個ずつの
    続きを押さずに最後まで実行します（破片化は取り消せないため**既定は OFF**です。
    途中で失敗したとき・窓を閉じたときは止まります）。
  * Mini Addons: アイテム破片化の**枠（X / Y）と「自動」を、破片化のウィンドウからも
    設定できる**ようにしました。設定画面へ戻らずに、一覧を見ながら詰め方を変えられます。
    上限（10）を超える数値を入れたときは、打ち直しを求めずに 10 へ丸めます。
  * Mini Addons: アイテム破片化の見出し（「フィルター｜イヤリング」など）が
    **2 行に折り返して下の行に被っていた**のを直しました。日本語ではどのタブ名も
    素の見出し幅に収まらないためで、全タブで 1 行に収まるようにしています。
  * Mini Addons: アイテム破片化で、**枠を広げるとスロットの下に大きな余白ができていた**
    のを直しました。列を増やすとスロットは横幅で小さくなるのに、高さだけ素の大きさで
    見積もっていたためです。
  * Mini Addons: アイテム破片化で、**絞り込みで一覧を作り直したときに選択を外す**
    ようにしました（作り直すとスロットには別の耳飾りが入り直すため、選択だけが位置に
    残っていると意図しない耳飾りを破片化してしまうおそれがありました）。

* **v2.7.0**
  * Another Warehouse: **設定ウィンドウ（歯車ボタン）を閉じると、チーム倉庫まで
    一緒に閉じてしまう**のを修正しました。設定を閉じる処理が、ゲーム側の「倉庫を
    閉じる」処理をそのまま呼んでいたためです（× でも ESC でも同じでした）。
    あわせて、**設定を閉じた後もインベントリの右クリックが「セットへ登録」のまま
    残っていた**のを、倉庫が開いていれば搬入へ、閉じていれば元へ戻すようにしました。
  * Another Warehouse: **お気に入りを追加しました**（yoma16版 new_nexus_addons v1.0.9 由来）。
    よく出すアイテムを**一覧の先頭に固定**できます。
    * 上部の **`★` ボタン**で編集の窓が開きます。**この窓を開いている間**、チーム倉庫か
      インベントリのアイテムを**右クリックすると足します**（窓を閉じれば、右クリックは
      いつもどおり搬出入に戻ります）。
    * 窓の中では **▲▼ で並べ替え**、**× で外せます**。並べた順がそのまま表示順です。
    * 一覧では **`お気に入り (n)` の帯が一番上**に出ます。**元のカテゴリからは消えません**
      ので、いつもの場所からも今までどおり取り出せます。検索でも一緒に絞り込まれます。
    * 左端のタブの**一番上に `★` のタブ**が増えます。押すとお気に入りだけを表示します。
    * 登録は**アカウント単位**なので、全キャラで共通です。
    * **倉庫を閉じると、この窓も一緒に閉じます。**（残したままだと、開き直したときに
      右クリックが搬出入ではなくお気に入りの登録のままになってしまうためです）
    * **検索で絞っている間に足しても、絞り込みは解けません。**
    * 窓は**初めて開いた位置を覚えます**。以前は足すたびに窓が少しずつ上へ跳ねていました。
    * 一覧の帯の見出しが「お気に入り」「お気に入り」と 2 段に出ていたのを 1 段にしました。
    * `TAKE SET` とは別のものです。あちらは「何を何個取り出すか」まで決めて実際に
      搬出する仕組みで、お気に入りは**並べ方を変えるだけ**です。自動の搬出入には
      関わりません。
  * Another Warehouse: タブが 10 個から **11 個**になったので、**タブの間隔を少し詰めました**
    （55px → 50px）。この枠の高さは倉庫の窓に合わせた固定値で伸ばせないためです。
    既存のタブの位置が少しずつ上へ寄ります。
  * Vakarine Equip: **設定画面を作り直しました**（yoma16版 new_nexus_addons v1.0.8 由来）。
    * **チェックを 2 列に並べました。**「作動する場所」8 個と「着脱する部位」13 個を
      1 列に積んでいたので縦に 21 行ぶん伸びていて、**1280x720 の画面では下が切れて
      いました**。
    * 部位に **「全部えらぶ / 全部はずす」ボタン**を足しました。今の状態で意味が
      切り替わるので、ボタンの文字を読めば次に何が起きるか分かります。
    * **設定画面をドラッグで動かせる**ようにし、**位置も覚える**ようにしました。
      これまでは動かすことすらできませんでした。チェックを押すたびに画面を組み直す
      作りなので、動かした位置はそのまま保ちます。
    * **背景を半透明**にしました。設定を見ながら装備の絵を確かめられます。
    * **窓の幅を広げました**（500 → 572）。チェックの文字は枠に収まるまで自動で縮むので、
      狭いと「インスタンスダンジョン（レイド/ミッション）」のような長い項目だけ
      極端に小さくなっていました。
    * **「フレームを固定」を「アイコンを固定」に直しました。** この設定が効くのは
      画面に出ている**小さなアイコンだけ**で、設定画面には効きません。
      何を固定するのかがツールチップにも出ます。
  * Indun Panel: **パネルの大きさを設定で選べる**ようにしました（`100%` / `90%` / `80%`）。
    設定の「パネル」タブ、背景の下に並びます。**座標だけでなく、ボタンの大きさ・アイコン・
    文字の大きさも一緒に縮みます**。画面に対してパネルが大きすぎると感じていた場合に
    使ってください。**既定は 100%（これまでどおり）**です。
    なお**設定ウィンドウの大きさは変わりません**。あちらは 1280x720 の画面にも収まるよう
    500 × 640 と決めてあり、タブでも中身の量でも変えない作りだからです
    （yoma16版 new_nexus_addons v1.0.7 では 80% 固定でしたが、こちらは選べるようにしました）。
  * Indun Panel: **設定に「入場券」タブを足しました。** 入場券をどの順に使うかを▲▼で
    決められます。並ぶのは **`期限付き` `取引不可` `購入` `取引可`** の 4 つで、
    上にあるものから順に試し、持っていないものは飛ばします。
    **レイド / チャレンジ・分裂特異点 / その他（ヴェルニケ・テルハルシャ・嘆きの墓地・
    共鳴の聖所）**の 3 つに分けて指定できます（レイドの券はショップに無いので「購入」は
    出ません）。**既定はこれまでどおりの順番**なので、触らなければ動きは変わりません。
    「取引不可の券は売れないのだから、買う前に使い切りたい」という人は、`取引不可` を
    `購入` の前へ上げてください。
  * Indun Panel: **どの入場券が使われるかが決まっていなかった**のを修正しました。
    2 つあります。**期限の無い券は、リストの最後に書かれたものが使われていました**
    （途中で上書きし続ける書き方になっていたためで、チャレンジの 540 と 560 で
    逆の動きになっていました）。**期限付きの券は「残り 1 日を切っているか」だけで
    並べ替えていた**ため、1 日券は残りがちょうど 24 時間で「切っていない」扱いになり、
    ほとんどの券が同点になっていました。同点の並びは決まっていないので、
    どれが使われるかは起動ごとに変わり得ます。今は**残りの短いものから**使い、
    同じなら書いてある順に使います。
    あわせて、**取引できるかどうかを ID の書き分けではなく所持品の実物から見る**ように
    しました（素のクライアントと同じ見方をします）。新しい段の券が増えたときに
    振り分けを間違えることがなくなります。
  * Indun Panel: 券を選ぶ処理が**レイド・チャレンジ・分裂特異点・ヴェルニケ・
    テルハルシャ・嘆きの墓地／共鳴の聖所で別々に書かれていた**のを 1 つにまとめました。
    上の修正はこの 6 か所すべてに効きます。
  * Indun Panel: **チャレンジ Lv560 で、片方のショップの購入枠を使い切っただけでは
    手持ちの期限なし券を使わない**ようにしました。この段は TOS コインと採掘場の
    2 つのショップを持つので、押したボタン側が空でも、もう片方に枠が残っていれば
    まだ買えます。あわせて、券を使ってしまうと採掘場の**追加購入枠（OverBuy）まで
    たどり着けなくなる**のも直しました。※「入場券」タブで `取引不可` や `取引可` を
    `購入` より**上**へ並べ替えている場合は、その指定どおり手持ちを先に使います。
  * Indun Panel: **鍵の掛かった入場券があると、実際には入場できているのに
    毎回「ロック」の注意が出ていた**のを、**何もできなかったときだけ**出すように
    しました。
  * Indun Panel: **テルハルシャの `BUYUSE` が、入場券を持っていても使わずに
    毎回ショップで買っていた**のを修正しました。使う側のアイテム ID が 1 桁多い
    誤記（実在しない ID）になっていたためです。ボタンのツールチップが数えていた
    ID のほうが正しく、これで**持っている券から先に使う**ようになります。
    （本家から引き継いだ誤りです。ゲーム本体のデータを引いて確認しました）
  * Muteki: **パーティーチャットへ知らせる設定にしたバフが切れると、ゲームが落ちていた**のを
    修正しました。バフが外れたことをゲーム内部が知らせている最中に、その処理の中から
    チャットを送っていたのが原因です。**文面はこれまでどおりその場で組み立てて、送信だけ
    0.1 秒後にずらしました**（見え方は変わりません）。ニコチャットと、バフが付いたときの
    知らせも同じ形にしています。※ゲーム本体側の異常終了なので、エラーの記録には
    何も残りません（yoma16版 new_nexus_addons v1.1.2 由来）。
  * Other Character Skill List: **Indun List Viewer で代表クラスを選んでいるキャラだけ、
    名前の左のクラスアイコンが空欄になっていた**のを修正しました。あわせて、クラスを
    引けなかったときは Indun List Viewer と同じ「?」の絵を出すようにしています
    （yoma16版 new_nexus_addons v1.1.0 由来）。
* **v2.6.1**
  * Mini Addons: 破片化の**「残す条件」のプリセットを、専用のファイルへ分けました**
    ...
    プリセットを使えます**（相手の同じ場所へ置くだけ。ゲームを終了してから
    置き換えてください）。これまでは Mini Addons の設定と同じファイルに入っていて、
    渡すと相手の他の設定まで上書きしてしまうため、そのままでは共有できませんでした。
    v2.6.0 で保存したプリセットは、条件の窓を次に開いたときに自動で移ります。
* **v2.6.0**
  * Vakarine Equip: **着脱の最後に、利用者が自分で開いていたインベントリまで
    閉じてしまう**のを修正しました。閉じるのはアドオンが自分で開いたときだけになります。
  * Vakarine Equip: 設定に**「着脱中にインベントリを開く」**を足しました（既定 ON =
    これまでどおり）。**OFF にすると着脱が始まるまでの待ち時間が短くなります。**
    インベントリはアイテム 1 個につき枠を 1 つ作るため、**持ち物が多い人ほど開くのに
    時間がかかっていました**（所持 1900 個の環境で 2.2 秒）。装備の要求はインベントリの
    窓を経由しないので、開かなくても着脱は行えます。
  * Guild Event Warp: 画面右上のボタン列が**インベントリより手前に描かれ、インベントリと
    重なった場所を押すとワープしてしまう**のを修正しました。素の画面端の小さなボタンと
    同じレイヤーへ下げ、インベントリなど普通のウィンドウの奥に置くようにしました。
  * Mini Addons: 「特性をスキル順に並べる」が、**特性の窓を閉じているときにも並べ替えを
    行っていた**のをやめました。素のクライアントは装備を替えるたびに特性の一覧を組み直す
    作りで、その通知を窓の開閉に関係なく受け取るため、装備を 1 部位替えるごとに
    見えていない窓の並べ替えが走っていました。装備を替えたときの引っかかりが減ります
    （窓を開いたときとタブを切り替えたときには従来どおり並べ替えます）。
  * Mini Addons: **「アイテム破片化の枠を拡張し、耳飾りを細かく絞り込む」を追加しました**
    ...
    * **スロットが 5x5 固定でなくなります。** 設定の右の入力に列と行（それぞれ 1〜10）を
      ...
      増やした枚数のまま押せます。**窓はクイックスロットより手前に出ます**
      （素の破片化はクイックスロットより奥にあり、行を増やすと下側のボタンが
      隠れてしまうため。OFF に戻すと元の奥行きに戻ります）。
    ...
    * **「条件で選択」ボタン**が付きます。**クラスごとに「この Lv 以上なら残す」**を決めて、
      そのどれにも当てはまらない耳飾りだけをまとめて選択できます（選ぶだけで、
      破片化そのものは今までどおり自分で実行ボタンを押します）。
      指定は**系統（ソードマン / ウィザード…）のタブ**を切り替えて、クラスの数字を
      **左クリックで上げ / 右クリックで下げ**るだけです（既定は全部 `-`）。
      枠は **`R1` `R2` `R3`**（ランク）の 3 つで、それぞれに「この Lv 以上なら残す」を
      入れます（ランクはツールチップの「[N] ランク スキルレベル」の N）。
      「R2 なら Lv3 以上、R3 なら Lv2 以上」のような書き方ができます。
      ランクを問わないときは 3 つとも同じ Lv にします。
      判定は**オプション 3 つのうち 1 つでも当てはまれば残す**、Lv は**その値以上**の意味です。
    * 指定は **名前を付けて保存でき、後から読み込めます**（プリセット）。
      キャラや用途ごとに持ち替えられます。同じ名前で保存すると上書きです。
