...
* **（次回リリース）**
  * Vakarine Equip: **着脱のときにインベントリを開かないようにしました**。開く必要が
    無かったためです（装備の要求はインベントリの窓を経由しません）。インベントリは
    アイテム 1 個につき枠を 1 つ作るので、**持ち物が多い人ほど、着脱が始まるまでの
    待ち時間が長くなっていました**（所持 1900 個の環境で、着脱が始まる前に 2.2 秒）。
    設定で「着脱中にインベントリを開く」を ON にすれば、従来どおり開きます。
  * Vakarine Equip: **装備を外す処理を速くしました**。これまでは「処理間隔」（既定 0.1 秒）
    ごとにしか外れたかどうかを見ておらず、外れていても次の確認まで待っていました。
    確認は細かく行い、外れた瞬間に次の部位へ進むようにしています（13 部位で
    2.6 秒かかっていたところが短くなります）。**「処理間隔」の意味は変えていません**
    （装備を着け直す間隔として、これまでどおり働きます）。
