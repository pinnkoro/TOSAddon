...
* **（次回リリース）**
  * **Characters Item Serch: OFF にしてあるのに、倉庫やチーム倉庫を閉じるたびに
    ...
    倉庫を開き直すと反映される）。
  * **Characters Item Serch: チーム倉庫の中身が検索に一度も出ていなかったのを修正。**
    チーム倉庫のアイテムはクライアント側に実体を持たないため 1 件も取得できず、
    記録が常に空になっていた（そのうえで上記のメッセージが出ていた）。
    アイテムの種類の名前で記録するようにして、チーム倉庫も検索できるようにした。
    個人倉庫でも同様に、実体を取得できなかったアイテムを取りこぼさなくなった。
