...
* **（次回リリース）**
  ...
  * Quickslot Operate: **マウスオーバーでの種族選択パネルが出なかったのを修正。**
    パネルを出すためのマウス判定が登録されておらず、登録してもスロットの
    作り直しで消えていた。併せて、パネルに並ぶ
    アイコンを所持しているポーションに合わせた（従来は旧シリーズの ID 固定だったため、
    新しいポーションしか持っていないと見慣れないアイコンが並んでいた）。
    ON/OFF を切り替えたときの解除も効くようにした。
