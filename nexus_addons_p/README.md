...
* **（次回リリース）**
  * Vakarine Equip: **街以外のすべてのマップで装備を着脱してしまう不具合を修正。**
    ...
    このチェックは今後 **ボス協同戦（JSR）のマップで着脱するかどうか**を決めます。
    ボス協同戦以外のインスタンスダンジョン（レイド / ミッション）はこの設定に関係なく
    従来どおり作動し、フィールドや旧ダンジョンでは作動しません。マップの判定は
    ID の直書きではなくマップの `Keyword`（`FieldBossMap`）で行うので、マップが
    追加されても漏れません。
    * **⚠ 動作が変わります。** ボス協同戦はインスタンスマップのため、これまでは
      **チェックの ON/OFF に関わらず必ず作動していました**（設定が実質効いていませんでした）。
      今後は **チェックが OFF だとボス協同戦では作動しません**。
      **既定は OFF なので、ボス協同戦で自動起動させたい方は設定画面でチェックを入れてください。**
  ...
  * Quickslot Operate: **ジョイスティック用のクイックスロットバーが消える不具合を修正。**
    ...
    **まず出さずに差し替えてみて、反映されなかったときだけ出す**ようにした。
    ...
    差し替えの前後でバーの表示状態と透明度を**実際に見てから元に戻す**ので、
  ...
  * Market Favorite Rebuild: **お気に入りが一度に全部消えるのを防ぐ歯止めを入れた。**
    ...
    2 件以上あったものが一度に 0 件になる保存だけを見送ります
    （1 個ずつ消していく操作はこれまでどおりです）。
