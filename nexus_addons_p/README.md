...
* **v2.6.0**
  ...
  * Mini Addons: **「アイテム破片化の枠を拡張し、耳飾りを細かく絞り込む」を追加しました**
    ...
    * プリセットは **`fragmentation_presets.json` という専用のファイル**に保存されます
      （`addons\_nexus_addons_p\<数字のフォルダ>\` の中）。他の設定とは別のファイルなので、
      **このファイルをそのまま渡せば、相手も同じプリセットを使えます**
      （相手の同じ場所へ置くだけ。ゲームを終了してから置き換えてください）。
      以前の版で保存したプリセットは、初めて開いたときにこのファイルへ移ります。
