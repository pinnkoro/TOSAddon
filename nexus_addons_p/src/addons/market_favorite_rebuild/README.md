...
売れずに戻ってきたものを、出品したときと**同じ単価・個数・販売期間**でもう一度出品します。
単価を打ち直さずに済ませるためのものです。

| 段 | 動き |
| --- | --- |
| 押したとき | 単価・個数・販売期間を出した確認ダイアログが出ます（**出品には手数料が掛かる**ため） |
| はいを押した後 | まず受け取り、アイテムがインベントリへ届いたのを確かめてから出品します |
| 届かなかったとき | 出品はせず、その旨をシステムメッセージで伝えます（インベントリが一杯などの場合） |
...
**同じアイテムの控えが 2 件以上あるときは、どの値段か決められないのでボタンを出しません**
（黙って別の値段で出し直すほうが害が大きいため）。受け取った後も同じ理由で、
