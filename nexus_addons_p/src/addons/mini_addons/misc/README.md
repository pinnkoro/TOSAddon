...
| [fragmentation.lua](fragmentation.lua) | `fragmentation` | アイテム破片化の枠を列 x 行で広げ、耳飾りを等級（1〜7 / 8 以上）と最大 Lv で絞り込む。「条件で選択」で、クラスごと・ランクごとに決めた「この Lv 以上なら残す」に当てはまらないものだけ選択する（指定は名前を付けて保存・読込でき、保存先は人に渡せるよう専用の fragmentation_presets.json） |
