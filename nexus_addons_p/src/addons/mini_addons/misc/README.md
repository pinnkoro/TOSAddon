...
| [fragmentation.lua](fragmentation.lua) | `fragmentation` | アイテム破片化の枠を列 x 行で広げ、耳飾りを等級（1〜7 / 8 以上）と最大 Lv で絞り込む。「条件で選択」で、クラスごとに決めた「この Lv（とランク）以上なら残す」に当てはまらないものだけ選択する（指定は名前を付けて保存・読込できる） |
