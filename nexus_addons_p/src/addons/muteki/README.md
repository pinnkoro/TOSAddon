...
![MUTEKI Settings の画面。上部に「指定されたバフの残り時間まで非表示(秒)」「レイヤーレベル」「モード切替」「フレームロック」「アイコン回転」と Add ボタンが並ぶ。その下に登録したバフが 1 つずつブロックで並び、各ブロックにバフ ID・アイコン・バフ名・色見本と、アイコンモードで表示などのチェックボックスが入る](images/02-settings.png)
