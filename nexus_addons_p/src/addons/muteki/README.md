...
![「エフェクト設定 : 叛天」の窓。上段は「バフ開始時」のチェックと種類のドロップリスト（通常エフェクト）、その下に「一覧から選ぶ」と「試し再生」、エフェクト名 F_pc_class_change と大きさ 1.5、「エフェクト中は透明度を自動で上げる」のチェックが並ぶ。下段は「重複が最大になったとき」で同じ並びになっていて、エフェクト名は F_pattern025_loop](images/03-effect.png)
