...
目で追い直さずに済みます。行数はコンテンツが増えるたびに伸びるため、窓の背丈ではなく
スクロールで受けています（コンテンツタブだけ高さ固定）。

**窓の幅は両タブとも同じ**です。タブごとに変えていると、窓を画面の右端へ寄せているときに
広いタブではみ出したぶん左へ戻され、狭いタブへ戻しても元の位置に復帰しません
（切り替えるたび少しずつ左へ寄る）。
...
* 派生版での主な修正:
  ...
  * 設定ウィンドウの**幅を両タブで揃えました**（右端へ寄せているとタブ切り替えのたびに
    窓が左へ寄っていくため）（次回リリース）
