...
![Easy Buff Config の画面。タイトルの右に「自動実行はキャラごと: ヒーラー」と今のキャラ名が出ている。preset 1〜4 が縦に並び、各プリセットの左端に自動実行の選択チェック（この例では preset 1 だけ ON）、右に料理 6 種類のチェックが横並びで付く。最下部にバフ掛け直し確認と修理屋フレームの自動クローズのチェックがある](images/02-settings.png)
