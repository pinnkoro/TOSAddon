...
![左がカード装着画面、右がカードプリセット画面。右側に 10 プリセットのドロップリストと SAVE / EQUIP / REMOVE のボタン、緑のカード群の下に「終了後に閉じる」チェックが追加されている。左側の色ごとのカード群には「保護」のラベルが付いたチェックボックスが並び、チェックを入れた赤と緑はラベルが黄色になってカードが灰色になっている](images/02-preset.png)

| 追加ボタン | 動作 |
| --- | --- |
| ドロップリスト | 10 個のプリセットから 1 つ選ぶ |
| **`終了後に閉じる`** | 動作が終わったらカード画面を自動で閉じる（既定は ON）。**外すと開いたまま残る**ので、結果を確かめられます |
...
実行中は `[MCC]動作中。バグ防止の為他の動作は行わないでください` と通知が出て、
**`SAVE` / `EQUIP` / `REMOVE`・プリセットの選択・色の保護チェックが押せなくなります**。
終わると `[MCC]End of Operation` と表示され、画面が自動で閉じます。

倉庫やインベントリの操作までは止めていません。画面全体を覆う作りにすると、
外し忘れたときにゲーム内から復旧できなくなる（クライアントの再起動しかなくなる）ためです。
