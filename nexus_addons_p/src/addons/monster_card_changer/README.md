...
![カードプリセット画面。右側に 10 プリセットのドロップリストと SAVE / EQUIP / REMOVE のボタンが追加され、左側の色ごとのカード群には保護用のチェックボックスが付く](images/02-preset.png)
...
* 適用は、ゲーム側のプリセット枠 **0 番を作業用に借りて**アドオンの内容を流し込み
  ...
  * 書き込みと適用は**同じ操作の中で続けて送ります**。以前は書き込みが 1 秒後の予約
    だったため、届く前に適用を押せてしまい、1 つ前に選んでいたプリセットが適用されて
    いました。
  * **`RequestCardPreset` で確かめ直してはいけません。** サーバーから取り直して
    キャッシュを上書きするので、今書いた内容が古い内容で消えます（実機で踏みました）。
    `SetCardPreset` は `GetCardPresetInfo` を同期で更新するので、読み戻しても
    確認できるのは手元の状態だけです。
