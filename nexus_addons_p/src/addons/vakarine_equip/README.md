...
**ウィンドウはドラッグで動かせ、位置も覚えます。** チェックを押すたびに中身を組み直す
作りですが、動かした位置はそのまま保ちます。

![Vakarine Equip の設定画面。半透明の背景に、上段「自動で作動する場所」の 8 チェックと下段「着脱する部位」の 13 チェックがそれぞれ 2 列で並ぶ。「着脱する部位」の見出しの右に「全部えらぶ / 全部はずす」ボタン、最下部にアイコンを固定・着脱中にインベントリを開く・アイコンを元の位置へが並ぶ](images/02-settings.png)

| 項目 | 既定値 | 説明 |
| --- | --- | --- |
| 自動で作動する場所（8 個） | 上の表のとおり | **チェックした場所でだけ**自動起動する |
| 各装備部位のチェック（13 個） | すべて OFF | **脱着する部位**。`RH_SUB` と `LH_SUB` は連動して切り替わる |
| アイコンを固定 | ON | **画面に出ている小さなアイコン**のドラッグ移動を禁止する（この設定ウィンドウには効かない） |
| 着脱中にインベントリを開く | ON | 着脱そのものには**必要ありません**。**持ち物が多い人は OFF にすると速くなります**（所持 1900 個の環境で、着脱が始まるまでに 2.2 秒かかっていました） |
| 全部えらぶ / 全部はずす | — | 部位のチェックをまとめて切り替える。**今の状態で意味が変わる**（1 つでも外れていれば「全部えらぶ」、全部 ON なら「全部はずす」） |
| アイコンを元の位置へ | — | アイコンの保存位置を破棄する |
...
`open_inventory` は着脱中にインベントリを開くかどうか（既定 1 = 開く。設定画面から変更できます）。

`x` / `y` は**アイコン**の位置、`config_x` / `config_y` は**設定ウィンドウ**の位置です。
どちらもドラッグで動かすと控えます。
