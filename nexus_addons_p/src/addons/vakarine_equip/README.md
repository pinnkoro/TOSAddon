...
2. マップが次のいずれか
   ...
   * `8022`（ヴェルニケ）
   * `11244`（未知の聖域 3F）
   * **週間ボスレイド（ボス協同戦）のマップ**で、設定の「JSR で作動」が ON のとき
3. 設定で**脱着する部位を 1 つ以上**チェックしている

手動実行（街以外での左クリック）は、マップの条件を無視して動きます
（部位を 1 つもチェックしていない場合は何もしません）。

週間ボスレイドかどうかは、マップ ID の直書きではなくマップの `Keyword`
（`WeeklyBossMap`）で判定します。運営側でマップが追加されても漏れません。
フィールドや旧ダンジョンでは、「JSR で作動」の状態にかかわらず自動起動しません。
...
| 項目 | 既定値 | 説明 |
| --- | --- | --- |
| チェックすると JSR で作動 | OFF | **週間ボスレイド（ボス協同戦）のマップでも**自動起動する |
