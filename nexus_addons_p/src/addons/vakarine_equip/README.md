...
2. マップが次のいずれか
   * **ボス協同戦（JSR）のマップ**で、設定の「JSR で作動」が **ON** のとき
   * それ以外のインスタンスマップ（**`11227`＝分裂は除く**）
   * `8022`（ヴェルニケ）
   * `11244`（未知の聖域 3F）
3. 設定で**脱着する部位を 1 つ以上**チェックしている

手動実行（街以外での左クリック）は、マップの条件を無視して動きます
（部位を 1 つもチェックしていない場合は何もしません）。

ボス協同戦かどうかは、マップ ID の直書きではなくマップの `Keyword`（`FieldBossMap`）
で判定します。運営側でマップが追加されても漏れません。
**ボス協同戦は「JSR で作動」が OFF なら自動起動しません**（インスタンスマップですが、
この設定が優先されます）。フィールドや旧ダンジョンでは、設定にかかわらず自動起動しません。

実機で確認したマップの `Keyword` は次のとおりです。通常のレイドは `IsRaidField` で、
`FieldBossMap` は付かないため区別できます。

| マップ | MapType | Keyword |
| --- | --- | --- |
| ボス協同戦 `field_d_cathedral_78_1` | Instance | `FieldBossMap` |
| ズメイ `Raid_Zmei` | Instance | `IsRaidField;NoTrade;NoWarp;HealControl` |
| ヴェルニケ `d_solo_dungeon` | Dungeon | `solo_dungeon` |
| フィールド `ep13_f_siauliai_1` | Field | `None` |
...
| 項目 | 既定値 | 説明 |
| --- | --- | --- |
| チェックすると JSR で作動 | OFF | **ボス協同戦（JSR）のマップで**自動起動する。OFF なら作動しない |
